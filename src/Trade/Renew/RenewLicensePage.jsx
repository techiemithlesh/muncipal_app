...
const RenewLicensePage = ({ navigation, route }) => {
  ...
  useEffect(() => {
    const fetchTaxData = async () => {
      try {
        ...
        const body = {
          ...
          firmEstablishmentDate: establishmentDate,
          areaInSqft: totalArea || '',
          isTobaccoLicense: 0,
          licenseForYears: licenseFor || '1',
        };
        ...
        setTaxData(response.data?.data || null);
      } catch (error) {
        console.error(
    ...
    // Only call if required fields are filled
    if (totalArea && licenseFor) {
      fetchTaxData();
    }
  }, [totalArea, establishmentDate, licenseFor]); // 👈 add dependencies
